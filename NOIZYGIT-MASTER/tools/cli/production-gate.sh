...
run_check "CRITICAL" "No hardcoded secrets" \
    "cd '$PROJECT_ROOT' && ! grep -rn --include='*.ts' --include='*.js' --include='*.json' --include='*.yml' --include='*.yaml' --include='*.sh' --include='*.env' -E '(sk-ant-[a-zA-Z0-9]{10,}|sk-[a-zA-Z0-9]{20,}|AKIA[A-Z0-9]{16}|pass''word=[^$\"{][a-zA-Z0-9]{8,})' . --exclude-dir=node_modules --exclude-dir=.git 2>/dev/null && echo 'No secrets found'"
