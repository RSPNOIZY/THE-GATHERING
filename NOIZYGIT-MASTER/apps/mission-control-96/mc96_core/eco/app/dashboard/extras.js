...
// Upgraded 2026-03-27 — April 17 target build

const ACCOUNT_ID = '2446d788cc4280f5ea22a9948410c355';
const GITHUB_ORG = 'NOIZY.AI Enterprise';

// ── ACTIVITY FEED ──────────────────────────────────────────
let feedEvents = [
  { icon:'⚡', msg:'GABRIEL v4 — multi-agent orchestration <strong>initializing</strong>', tag:'build', time:'just now' },
  { icon:'🌙', msg:'DreamChamber v2 architecture in progress — <strong>:7777</strong>', tag:'build', time:'2m ago' },
  { icon:'🎙️', msg:'voice-v2 pipeline scaffolded — STT/Claude dispatch <strong>pending deploy</strong>', tag:'voice', time:'4m ago' },
  { icon:'✅', msg:'GitHub restructure complete — personal repos → <strong>ARCHIVE/</strong>', tag:'build', time:'5m ago' },
  { icon:'🔒', msg:'GitHub org: <strong>NOIZY.AI Enterprise only</strong> — no public repos', tag:'build', time:'6m ago' },
  { icon:'⚡', msg:'Cloudflare Worker <strong>claude-proxy</strong> ready to deploy', tag:'cf', time:'8m ago' },
  { icon:'🎙️', msg:'Logitech USB mic detected — Voice Command active', tag:'voice', time:'10m ago' },
  { icon:'🌙', msg:'Dream Chamber crew initialized — 6 members online', tag:'crew', time:'12m ago' },
  { icon:'🗄️', msg:'D1 schema ready: <strong>agent-memory / 7b813205</strong>', tag:'cf', time:'15m ago' },
  { icon:'☁️', msg:'Account ID <code>2446d788…</code> locked into Config', tag:'cf', time:'18m ago' },
];
...
function renderFeed() {
  ...
  if (!el) return;
  el.innerHTML = '';
  feedEvents.slice(0, 12).forEach(e => {
...
function addFeedEvent(icon, msg, tag) {
  ...
  feedEvents.forEach((e, i) => { if (i > 0) e.time = `${i * 2}m ago`; });
...
const workers = [
  ...
  { icon:'🌙', name:'dreamchamber-v2-worker', desc:'Multi-agent orchestration · :7777 bridge',       status:'ready' },
...
function deployWorker(name, btn) {
  ...
  btn.disabled = true;
  addFeedEvent('⚡', `Deploying <strong>${name}</strong> to Cloudflare edge…`, 'cf');
...
function renderCharts() {
  renderBar('chartCF',    [30,55,42,70,85,62,91], 'neon');
  renderBar('chartVoice', [10,22,15,40,35,52,48], 'cyan');
  renderBar('chartCrew',  [80,62,92,71,97,86,100],'gold');
}
...
// ── TERMINAL ───────────────────────────────────────────────
const termLines = [
  { text: '⛅  NOIZY.AI Cloudflare Worker CLI — Wrangler 4.x', cls: 'info' },
  { text: '─────────────────────────────────────────────', cls: 'dim' },
  { text: `✓  CF Account: ${ACCOUNT_ID}`, cls: '' },
  { text: '✓  Workers project: noizy-claude-proxy', cls: '' },
  { text: '✓  D1: agent-memory / 7b813205-fd12-4a23-84a6-ce83bc49ec70', cls: '' },
  { text: '✓  R2 bucket: noizy-voice-library [READY]', cls: '' },
  { text: '✓  DreamChamber: localhost:7777 [GABRIEL V4]', cls: '' },
  { text: '─────────────────────────────────────────────', cls: 'dim' },
  ...
  { text: '─────────────────────────────────────────────', cls: 'dim' },
  { text: '⚠  wrangler login required before first deploy', cls: 'warn' },
  { text: '⚠  CF Tunnel not yet deployed (voice.noizy.ai)', cls: 'warn' },
  { text: '→  Next: voice-v2 pipeline · DreamChamber v2 · April 17', cls: 'dim' },
];
...
function renderGitStatus() {
  ...
  el.innerHTML = `
    <div style="display:flex;align-items:center;gap:14px;padding:14px 18px;border-radius:12px;background:rgba(52,211,153,.04);border:1px solid rgba(52,211,153,.25)">
      ...
      <div style="flex:1">
        <div style="font-size:12px;font-weight:700;color:var(--green)">GitHub Enterprise Org · Restructure Complete · Enterprise-Only Policy</div>
        <div style="font-size:10px;color:var(--muted);margin-top:3px;line-height:1.5">
          All personal repos → <strong style="color:var(--green)">ARCHIVE/</strong> inside <strong style="color:var(--green)">NOIZY.AI Enterprise</strong>.
          No public repos. Enterprise org is the single source of truth.
        </div>
      </div>
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const liveEvents = [
    ...
    { icon:'✦',  msg:'Claude Max processing strategy request', tag:'crew' },
    { icon:'⌨️', msg:'Claude Code: Worker build in progress', tag:'build' },
    { icon:'🌙', msg:'DreamChamber v2 — agent sync pulse', tag:'build' },
    { icon:'💰', msg:'Royalty record queued in D1 agent-memory', tag:'cf' },
    { icon:'⚡', msg:'GABRIEL v4 — standing orders refreshed', tag:'build' },
  ];
  setInterval(() => {
