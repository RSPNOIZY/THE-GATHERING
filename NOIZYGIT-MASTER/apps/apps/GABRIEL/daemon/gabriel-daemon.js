...
for (const p of sqlitePaths) {
  try {
    ...
    db.exec(`
      ...
      -- ── V3.0 Command Spine Tables (Milestone #1) ─────────────────────────────
      ...
      CREATE TABLE IF NOT EXISTS ideas (
        ...
        originating_device TEXT NOT NULL,
        actor TEXT NOT NULL,
        created_at TEXT DEFAULT (datetime('now'))
      );
...
// ── V3.0 Command Spine & Rule Zero Helpers ─────────────────────────────────────
const commandStore = [];
const planStore = [];
const receiptStore = [];
const ideaStore = [];
...
function validateCommand(cmd) {
  ...
  const required = ['command_id', 'actor', 'source', 'intent', 'parameters', 'risk_class', 'timestamp', 'idempotency_key'];
  for (const field of required) {
    if (cmd[field] === undefined) return { valid: false, error: `Missing required field: ${field}` };
  }
  if (typeof cmd.command_id !== 'string') return { valid: false, error: 'command_id must be a string' };
  if (!['RSP_001', 'GABRIEL', 'LUCY', 'ALEX_WARD'].includes(cmd.actor)) return { valid: false, error: `Invalid actor: ${cmd.actor}` };
  if (!['Notion', 'Siri', 'Discord', 'Slack', 'iPad', 'iPhone', 'MICHAEL'].includes(cmd.source)) return { valid: false, error: `Invalid source: ${cmd.source}` };
  if (!['record_idea', 'search_catalog', 'send_message', 'start_car_mode', 'archive_audio', 'run_backup', 'deploy', 'system_health'].includes(cmd.intent)) return { valid: false, error: `Invalid intent: ${cmd.intent}` };
  if (typeof cmd.parameters !== 'object' || cmd.parameters === null) return { valid: false, error: 'parameters must be an object' };
  if (!['low', 'medium', 'high', 'critical'].includes(cmd.risk_class)) return { valid: false, error: `Invalid risk_class: ${cmd.risk_class}` };
  if (typeof cmd.timestamp !== 'string') return { valid: false, error: 'timestamp must be a string' };
  if (typeof cmd.idempotency_key !== 'string') return { valid: false, error: 'idempotency_key must be a string' };
  return { valid: true };
}

async function processCommandRuleZero(cmd) {
  // Step 1: Validate Schema
  const val = validateCommand(cmd);
  if (!val.valid) {
    throw new Error(`Validation Failed: ${val.error}`);
  }

  // Step 2: Authorize (Rule Zero check)
  if (cmd.risk_class === 'critical' && cmd.actor !== 'RSP_001') {
    throw new Error(`Rule Zero Violation: Critical risk actions require RSP_001 authority.`);
  }

  // Check idempotency (prevent replay)
  if (db) {
    const existing = db.prepare("SELECT command_id FROM commands WHERE idempotency_key = ?").get(cmd.idempotency_key);
    if (existing) {
      throw new Error(`Duplicate Command: Idempotency check failed for key ${cmd.idempotency_key}`);
    }
  } else {
    const existing = commandStore.find(x => x.idempotency_key === cmd.idempotency_key);
    if (existing) {
      throw new Error(`Duplicate Command: Idempotency check failed for key ${cmd.idempotency_key}`);
    }
  }

  // Log command to db or store
  if (db) {
    db.prepare(`
      INSERT INTO commands (command_id, actor, source, intent, parameters, risk_class, timestamp, idempotency_key, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'authorized')
    `).run(
      cmd.command_id,
      cmd.actor,
  ...
  // Step 3: Immutable Bounded Plan
  const plan_id = crypto.randomUUID();
  let steps = [];

  if (cmd.intent === 'record_idea') {
    steps = [
      { step: 1, worker: 'CB01', action: 'validate_idea_content', parameters: cmd.parameters },
      { step: 2, worker: 'CB01', action: 'save_idea_locally', parameters: cmd.parameters },
      { step: 3, worker: 'GABRIEL', action: 'dispatch_notion_sync', parameters: cmd.parameters }
    ];
  } else {
    steps = [
      { step: 1, worker: 'CB02', action: 'execute_stub', parameters: cmd.parameters }
    ];
  }

  if (db) {
    db.prepare(`
      INSERT INTO plans (plan_id, command_id, steps, status)
      VALUES (?, ?, ?, 'created')
    `).run(plan_id, cmd.command_id, JSON.stringify(steps));
  } else {
    planStore.push({
      plan_id,
      command_id: cmd.command_id,
  ...
  try {
    if (db) {
      db.prepare("UPDATE plans SET status = 'running' WHERE plan_id = ?").run(plan_id);
    } else {
      const p = planStore.find(x => x.plan_id === plan_id);
      if (p) p.status = 'running';
    }

    for (const step of steps) {
      log(`Executing Plan Step ${step.step} via worker ${step.worker}: ${step.action}`);
      if (step.action === 'save_idea_locally') {
        const idea_id = crypto.randomUUID();
        const idea_text = step.parameters.idea || "";
        if (!idea_text) throw new Error("Idea text cannot be empty");
        if (db) {
          db.prepare(`
            INSERT INTO ideas (id, command_id, idea_text, originating_device, actor)
            VALUES (?, ?, ?, ?, ?)
          `).run(idea_id, cmd.command_id, idea_text, cmd.source, cmd.actor);
        } else {
          ideaStore.push({
            id: idea_id,
            command_id: cmd.command_id,
            idea_text,
            originating_device: cmd.source,
            actor: cmd.actor,
            created_at: new Date().toISOString()
          });
        }
        log(`Idea saved locally to 'ideas' table`);
      } else if (step.action === 'dispatch_notion_sync') {
        try {
          const syncMockPath = path.join(__dirname, 'notion_sync_mock.js');
          if (fs.existsSync(syncMockPath)) {
            const { syncCommand } = require(syncMockPath);
            syncCommand(cmd, plan_id);
          }
        } catch (e) {
    ...
    if (db) {
      db.prepare("UPDATE plans SET status = 'completed' WHERE plan_id = ?").run(plan_id);
      db.prepare("UPDATE commands SET status = 'completed' WHERE command_id = ?").run(cmd.command_id);
    } else {
      const p = planStore.find(x => x.plan_id === plan_id);
      if (p) p.status = 'completed';
      const c = commandStore.find(x => x.command_id === cmd.command_id);
    ...
    if (db) {
      db.prepare("UPDATE plans SET status = 'failed' WHERE plan_id = ?").run(plan_id);
      db.prepare("UPDATE commands SET status = 'failed' WHERE command_id = ?").run(cmd.command_id);
    } else {
      const p = planStore.find(x => x.plan_id === plan_id);
  ...
  const secretKey = API_KEY || "noizy-default-secret-key-signature-seed";
  const hmac = crypto.createHmac('sha256', secretKey);
  hmac.update(`${cmd.command_id}:${plan_id}:${status}`);
  const signature = hmac.digest('hex');

  if (db) {
    db.prepare(`
      INSERT INTO receipts (receipt_id, command_id, plan_id, status, signature)
      VALUES (?, ?, ?, ?, ?)
    `).run(receipt_id, cmd.command_id, plan_id, status, signature);
  } else {
    receiptStore.push({
      receipt_id,
      command_id: cmd.command_id,
  ...
  return { plan_id, receipt, error: executionError };
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (method === "POST" && url === "/api/v1/commands") {
    let body;
    try {
    ...
    try {
      const result = await processCommandRuleZero(body);
      if (result.error) {
        send(res, 500, { error: "Execution failed", detail: result.error, receipt: result.receipt }, req);
      } else {
        send(res, 200, { ok: true, plan_id: result.plan_id, receipt: result.receipt }, req);
      }
    } catch (e) {
      send(res, 400, { error: e.message }, req);
    }
    return;
  }

  // ── GET /api/v1/commands/verify ──────────────────────────────────────────
  if (method === "GET" && url === "/api/v1/commands/verify") {
    const parsedUrl = new URL(req.url, `http://${req.headers.host || 'localhost'}`);
    const command_id = parsedUrl.searchParams.get("command_id");
    if (!command_id) {
      send(res, 400, { error: "command_id required" }, req);
      return;
    }
    if (db) {
      try {
        const command = db.prepare("SELECT * FROM commands WHERE command_id = ?").get(command_id);
        const plan = db.prepare("SELECT * FROM plans WHERE command_id = ?").get(command_id);
        const receipt = db.prepare("SELECT * FROM receipts WHERE command_id = ?").get(command_id);
        const idea = db.prepare("SELECT * FROM ideas WHERE command_id = ?").get(command_id);
        send(res, 200, { ok: true, store: "sqlite", command, plan, receipt, idea }, req);
      } catch (e) {
        send(res, 500, { error: e.message }, req);
      }
    } else {
      const command = commandStore.find(x => x.command_id === command_id);
      const plan = planStore.find(x => x.command_id === command_id);
      const receipt = receiptStore.find(x => x.command_id === command_id);
      const idea = ideaStore.find(x => x.command_id === command_id);
      send(res, 200, { ok: true, store: "in-memory", command, plan, receipt, idea }, req);
    }
    return;
  }
