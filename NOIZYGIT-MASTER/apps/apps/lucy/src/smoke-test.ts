...
test('Converts report to n8n feed', () => {
  const report = runNightlyAnalysis();
  const feed = convertToN8nFeed(report);
...
test('Actions sorted by priority', () => {
  const report = runNightlyAnalysis();
  const feed = convertToN8nFeed(report);
...
test('Only compassion-cleared opportunities generate actions', () => {
  const report = runNightlyAnalysis();
  const feed = convertToN8nFeed(report);
...
test('All actions have governance fields', () => {
  const report = runNightlyAnalysis();
  const feed = convertToN8nFeed(report);
  for (const action of feed.actions) {
