...
**Version:** 2.0 (2026-04-18) — superseded by `ops/DNS_CORRECTNESS_PLAN.md`
**Operator:** RSP_001 + GABRIEL
**Scope:** 3 GoDaddy domains (noizy.ai, noizyfish.com, fishmusicinc.com) + consolidate noizyvox.com to NOIZYFISH CF account
**Not registered (removed from all plans):** noizyfish.ca, noizylab.ca, noizlab.ca

> **This runbook is now a detail companion to `ops/DNS_CORRECTNESS_PLAN.md`.** The plan is canonical; this document expands click-by-click UI steps.
...
| 3    | Check noizy.ai                | Active, GoDaddy, expires 2027-09-26 — **RENEW +2y** |
| 4    | Check noizyfish.com           | Active, expires 2027-08-16                          |
| 5    | Check fishmusicinc.com        | Active, expires 2031-01-27                          |
| 6    | Note expiration dates for all | Record in table below                               |

**Record:**

| Domain           | Status | Expires                      | Lock Status | EPP Code |
| ---------------- | ------ | ---------------------------- | ----------- | -------- |
| noizy.ai         | Active | 2027-09-26 → renew +2y first |             |          |
| noizyfish.com    | Active | 2027-08-16                   |             |          |
| fishmusicinc.com | Active | 2031-01-27                   |             |          |

> **.ai rule:** registry requires ≥2 years remaining for transfer. noizy.ai currently has ~1.4y. Renew at GoDaddy for +2y BEFORE requesting the EPP code. Run `bash ops/cf-transfer-preflight.sh noizy.ai` to confirm clearance.
...
| 5    | Confirm noizykidz.com shows "Active" (NOIZYFISH)                    | Green checkmark |
| 6    | Confirm noizyvox.com shows "Active" (Fishmusicinc — to consolidate) | Green checkmark |
...
| 2    | **fishmusicinc.com first** (lowest-risk dress rehearsal)    | Code accepted       |
| 3    | **noizyfish.com second** (coordinate — public email domain) | Code accepted       |
| 4    | **noizy.ai last** (only after +2y renewal clears preflight) | Code accepted       |
| 5    | Confirm contact: rsplowman@icloud.com                       | Correct             |
| 6    | Payment method on file (required)                           | Card on file        |
| 7    | Confirm all transfers                                       | Transfers initiated |

**Cost:**

- .com: ~$10.11/yr each (× 2 = $20)
- .ai: ~$20/yr × 2y minimum = $40 + ~$200 for pre-transfer +2y renewal at GoDaddy
...
**Timeline:** .com transfers: 5-7 days (often hours if approved quickly). .ai: up to 10 days.
...
# Preferred path: use ops/cf-dns-bootstrap.sh (idempotent, handles DMARC+CAA+Email Routing)
for domain in noizy.ai noizyfish.com fishmusicinc.com noizykidz.com noizyvox.com; do
  bash ops/cf-dns-bootstrap.sh --zone "$domain"
done

# Verify SPF (auto-added by CF Email Routing)
for domain in noizy.ai noizyfish.com fishmusicinc.com noizykidz.com noizyvox.com; do
  echo "=== $domain SPF ==="
