...
### 8. Wire Kill Switch to production ✅ DONE — 2026-04-17
- **Action:** ~~Expose `/kill-switch` endpoint on Heaven; require `X-NOIZY-Key` + actor signature.~~
- **DONE:** `POST /api/kill-switch` live in `src/index.ts`. Requires `X-Noizy-Key`. Flips `is_active=0` in `hvs_consent_tokens`, fires Gabriel audit, fires Slack `#noizy-alerts`.
- **Pending:** Set `SLACK_WEBHOOK_URL` secret: `wrangler secret put SLACK_WEBHOOK_URL` then `wrangler deploy`.
...
### 16. Slack webhook for CRITICAL events ✅ DONE — 2026-04-17
- **Action:** ~~Add `WEBHOOK_URL` secret to Heaven; fire on kill-switch + Never-Clause violation.~~
- **DONE:** `alertSlack()` helper live in `src/index.ts`. Fires on kill-switch + `never_clause_violation` hook events. `SLACK_WEBHOOK_URL` documented in `wrangler.toml` secrets.
- **Pending:** `wrangler secret put SLACK_WEBHOOK_URL` → `wrangler deploy` from Terminal.
