...
| **17 MCP Servers**                                   | LIVE     | `mcp/` — audio, cb01-mcp, consent-oracle, dream-mcp, dreamchamber-audio, engr-keith-mcp, family-mcp, gabriel-mcp, gemma3, heaven-mcp, lucy-mcp, metabeast-remote, shirley-mcp, shortcuts-mcp, supersonic, synthesis-oracle, voice-bridge (verified 2026-04-20) |
...
**[DEVICES.md](./DEVICES.md)** — single source of truth for every physical device, local-service port, MCP server, CF Worker, and agent voice. If a name, port, or ID disagrees with DEVICES.md, DEVICES.md wins.
...
| `mcp-builder.md`        | MCP build rules — local stdio, Python/FastMCP, Cloudflare remote; hostname policy; wrangler.jsonc standard; remote test flow |
