...
### 🔊 NOIZYVOX (NOIZYLAB legacy)
| Field | Value |
|-------|-------|
| **Name** | `noizy-vox` |
| **File** | `NOIZYANTHROPIC/NOIZYLAB/noizyvox/wrangler.toml` → `src/index.ts` |
| **CF Account** | `5f36aa9795348ea681d0b21910dfc82a` |
| **Route (prod)** | `vox.noisy.io/*` (custom domain) |
| **DB Binding** | `DB` → `noizy-vox` (`ea110bf3-5ad0-4dd3-ae3a-2ef05fc052ac`) |
...
| **DB Binding** | `DB` → `noisybox` (`PLACEHOLDER_NOIZYBOX_DB_ID` — ⚠️ CREATE NEEDED) |
| **Service Binding** | `PROOF` → `noisyproof` |
...
| `vox-noizy-ai` | `VOX_DB` | `noizy-vox` (`ea110bf3-5ad0-4dd3-ae3a-2ef05fc052ac`) | Voice ops |
...
| `noizy-vox` | `ea110bf3-5ad0-4dd3-ae3a-2ef05fc052ac` | noizy-vox, vox-noizy-ai | ✅ Active |
...
| `noisybox` | `PLACEHOLDER_NOIZYBOX_DB_ID` | noisybox worker | ⚠️ NEEDS CREATION |
...
| 7 | **NOIZYFISH/NOIZYBOX D1 IDs missing** | `wrangler d1 create noizyfish` + `wrangler d1 create noisybox` | Terminal |
...
├── NOIZYLAB/
│   ├── noizyvox/wrangler.toml        ← NOIZYVOX worker
│   ├── noisyproof/wrangler.toml      ← NoisyProof worker
│   ├── noizyfish/wrangler.toml       ← NOIZYFISH worker
│   └── noisybox/wrangler.toml        ← NOIZYBOX worker
├── repos/
...
│   ├── apps/noizyfish/               ← NOIZYFISH Next.js app
│   ├── apps/noizyvox/                ← NOIZYVOX Next.js app
