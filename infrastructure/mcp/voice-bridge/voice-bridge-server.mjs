...
const TOWERS = {
  max: {
    ...
    system: "You are Claude Max, strategic lead of NOIZY.AI Dream Chamber. Robert Stephen Plowman of Ottawa, Ontario, Canada is your founder. NOIZY.AI builds a premium voice library fighting for fair compensation for AI and human voice actors. Be direct and strategic.",
  },
  code: {
    model: "claude-sonnet-4-5",
    system: "You are Claude Code for NOIZY.AI. Build requests come via voice pipeline from Robert on iPhone → M2 Ultra. Give code, commands, concrete steps. Be concise.",
  },
  work: {
    model: "claude-sonnet-4-5",
    system: "You are Claude Coworker for NOIZY.AI Dream Chamber. Coordinate crew, delegate tasks, keep things moving. Voice input from Robert via iPhone.",
  },
};
...
server.tool(
  ...
  {
    command: z.enum(["gabriel", "claude", "deploy", "dreamchamber", "status"]).describe("Voice command to execute"),
    text: z.string().optional().describe("Text input for commands that need it (gabriel, claude)"),
    tower: z.enum(["max", "code", "work"]).optional().describe("Claude tower selection (default: auto-detect)"),
  },
  async ({ command, text, tower }) => {
    ...
    switch (command) {
      case "gabriel": {
        if (!text) return { content: [{ type: "text", text: "Error: gabriel command requires text input" }] };
        const result = await callGabriel(text);
        return { content: [{ type: "text", text: result }] };
      }

      case "claude": {
        if (!text) return { content: [{ type: "text", text: "Error: claude command requires text input" }] };
      ...
      case "deploy": {
        const result = await runCommand("cd /Users/m2ultra/NOIZYANTHROPIC && npx wrangler deploy --config noizy-landing/wrangler.toml 2>&1", 60000);
        return { content: [{ type: "text", text: `Deploy result:\n${result}` }] };
      }

      case "dreamchamber": {
        const result = await runCommand("cd /Users/m2ultra/NOIZYLAB/dreamchamber && docker compose up -d 2>&1");
      ...
      case "status": {
        ...
        try { localStatus = await runCommand("docker ps --format '{{.Names}}: {{.Status}}' 2>/dev/null; pm2 status 2>/dev/null || true"); }
        catch { localStatus = "docker/pm2 not running"; }
        return {
          content: [{
            type: "text",
...
server.tool(
  ...
  "Send a message directly to a specific Claude tower (Max, Code, or Coworker)",
  {
    ...
    tower: z.enum(["max", "code", "work"]).default("max").describe("Tower: max (strategy), code (build), work (crew tasks)"),
  },
  async ({ text, tower }) => {
...
server.tool(
  ...
  async () => {
    ...
    // Local services
    try {
      const n8n = await fetch("http://localhost:5678/healthz", { signal: AbortSignal.timeout(3000) });
      checks.n8n = { status: n8n.ok ? "running" : "error", code: n8n.status };
    } catch { checks.n8n = { status: "not running" }; }

    try {
      const ollama = await fetch("http://localhost:11434/api/tags", { signal: AbortSignal.timeout(3000) });
      const data = await ollama.json();
      checks.ollama = { status: "running", models: data.models?.length || 0 };
    } catch { checks.ollama = { status: "not running" }; }

    // Git
    try {
      const branch = await runCommand("cd /Users/m2ultra/NOIZYANTHROPIC && git branch --show-current");
      const status = await runCommand("cd /Users/m2ultra/NOIZYANTHROPIC && git status --porcelain | wc -l");
      checks.git = { branch, uncommitted_files: parseInt(status.trim()) };
    } catch { checks.git = { error: "not a git repo" }; }
    ...
    } catch { checks.hardware = { error: "unknown" }; }
...
server.tool(
  ...
  {
    endpoint: z.string().describe("API path, e.g. /api/v1/actors, /api/v1/stats, /health, /api/v1/ledger"),
  },
  async ({ endpoint }) => {
...
server.tool(
  ...
  {
    script: z.enum([
      "smoke-test", "full-status", "deploy-readiness", "ethics-gate",
      "empire-boot", "production-gate", "wrangler-doctor",
