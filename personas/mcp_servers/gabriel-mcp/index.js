...
function ensureState() {
  ...
  if (!existsSync(WATCHLIST_FILE)) {
    writeFileSync(WATCHLIST_FILE, JSON.stringify({ watches: [], last_check: null }, null, 2));
  }
}
...
async function apiCall(method, path, body = null) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`DreamChamber ${method} ${path} → ${res.status}: ${text}`);
  }
  return res.json();
}
...
const CONVERSATION_TYPES = {
  ...
  problemsolving: "Problem-solving — hypothesis, test, result cycles",
  creative: "Creative exploration — ideas, possibilities, art",
  operational: "Operational status — system health, monitoring",
  walkandtalk: "Walk and talk — mobile, fluid, thinking out loud",
  dreamchamber: "Dream Chamber — designing complete systems in isolation",
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      ...
      description: "Speak text aloud via macOS TTS (GABRIEL voice). No AI generation — pure TTS.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description: "Force GABRIEL to refresh its live context from the Heaven consent kernel.",
      inputSchema: { type: "object", properties: {} },
    },
      ...
      inputSchema: {
        ...
        properties: {
          ...
          participants: {
            type: "array",
            items: { type: "string" },
            description: 'Who is in this conversation: ["rob", "gabriel", "claude_1", etc.]',
          },
          tags: {
            type: "array",
            items: { type: "string" },
            description: "Context tags: domain, project, topic keywords",
          },
          context: {
            type: "string",
            description: "Additional context: where is Rob, what triggered this, related threads",
          },
        },
        required: ["title"],
      },
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          metadata: {
            type: "object",
            description: "Optional: tone, confidence, model used, source device, etc.",
          },
        },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          summary: {
            type: "boolean",
            description: "If true, return summary instead of full transcript",
          },
        },
        required: ["thread_id"],
      },
    ...
    },
    {
      ...
      description: "Search conversation threads by keyword, date, tag, participant, or type.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          type: {
            type: "string",
            enum: ["health", "repo", "file", "queue", "deploy", "conversation", "custom"],
            description: "What kind of watch",
          },
          alert_on: {
            type: "string",
            description: "When to alert: 'change', 'error', 'threshold', 'always'",
          },
          note: {
            type: "string",
  ...
  try {
    switch (name) {
      ...
      case "gabriel_status": {
        ...
        try {
          activeThreads = readdirSync(CACHE_DIR).filter((f) => f.endsWith(".json")).length;
          handoffPending = readdirSync(HANDOFF_DIR).filter((f) => f.endsWith(".json")).length;
        } catch {
          /* empty */
        }
      ...
      case "gabriel_refresh": {
        try {
          const result = await apiCall("POST", "/api/gabriel/refresh");
          return {
            content: [
              {
                type: "text",
      ...
      case "gabriel_cache_start": {
        ...
        const thread = {
          ...
          type_description: CONVERSATION_TYPES[args.type || "operational"],
      ...
      case "gabriel_cache_append": {
        ...
        if (!existsSync(threadFile)) {
          // Check handoff dir
          const handoffFile = join(HANDOFF_DIR, `${args.thread_id}.json`);
        ...
        if (
          ...
          thread.extracted.decisions[thread.extracted.decisions.length - 1].message_index ===
            message.index
        )
          extractions.push("decision detected");
        if (
          thread.extracted.action_items.length > 0 &&
          thread.extracted.action_items[thread.extracted.action_items.length - 1].message_index ===
            message.index
        )
          extractions.push("action item detected");
        if (
          thread.extracted.rule_candidates.length > 0 &&
          thread.extracted.rule_candidates[thread.extracted.rule_candidates.length - 1]
            .message_index === message.index
        )
      ...
      case "gabriel_cache_snapshot": {
        ...
        if (args.thread_id === "latest") {
          ...
          threadFile = join(CACHE_DIR, `${args.thread_id}.json`);
          if (!existsSync(threadFile)) threadFile = join(HANDOFF_DIR, `${args.thread_id}.json`);
        }
        ...
        const lines = [
          ...
          `── TRANSCRIPT ──`,
          ...thread.messages.map(
            (m) => `[${m.timestamp.slice(11, 19)}] **${m.speaker}:** ${m.content}`,
          ),
          ``,
          `── EXTRACTED ──`,
      ...
      case "gabriel_cache_handoff": {
        ...
        if (threadId === "latest") {
          ...
          if (files.length === 0) {
            return {
              content: [{ type: "text", text: "No active threads to hand off." }],
            };
          }
          threadFile = join(CACHE_DIR, files[0]);
        ...
        thread.extract_rules = args.extract_rules !== false;
          ...
          try {
            ...
            for (const file of files) {
              ...
              try {
                ...
                if (args.type && thread.type !== args.type) continue;
                if (args.participant && !(thread.participants || []).includes(args.participant))
                  continue;
                if (args.tag && !(thread.tags || []).includes(args.tag)) continue;
                if (args.date_from && thread.date < args.date_from) continue;
                if (args.date_to && thread.date > args.date_to) continue;
                if (
                  args.keyword &&
                  !JSON.stringify(thread).toLowerCase().includes(args.keyword.toLowerCase())
                )
                  continue;
      ...
      case "gabriel_watch_clear": {
        const data = readJson(WATCHLIST_FILE);
        const idx = data.watches.findIndex((w) => w.id === args.watch_id);
        if (idx === -1) {
          return {
            content: [
              {
