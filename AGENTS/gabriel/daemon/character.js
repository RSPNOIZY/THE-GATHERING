...
const MEMORY_DIR = process.env.GABRIEL_MEMORY_DIR ||
...
export function loadCreativeEcosystem() {
...
export function loadDreamChamber() {
...
export function loadDiscordFleet() {
...
export function buildSystemPrompt() {
    ...
    if (heaven)
        parts.push(heaven);
    if (neverClausesOps)
        parts.push(neverClausesOps);
    if (safetyContract)
        parts.push(safetyContract);
    if (mobileContinuity)
        parts.push(mobileContinuity);
    if (creativeEcosystem)
        parts.push(creativeEcosystem);
    if (dreamChamber)
        parts.push(dreamChamber);
    if (integrations)
        parts.push(integrations);
    if (discordFleet)
        parts.push(discordFleet);
    if (memory)
        parts.push(memory);
